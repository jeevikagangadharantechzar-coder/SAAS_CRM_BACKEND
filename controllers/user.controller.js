      ...
      const tempToken = jwt.sign(
        {
          ...
          sessionId: deviceSlot.sessionId
        },
        process.env.SECRET_KEY,
        { expiresIn: "5m" }
      );
      return res.json({ success: true, mfaRequired: true, tempToken });
    }
...
const verifyMfaLogin = async (req, res) => {
  try {
    ...
    if (!tempToken || !mfaCode) {
      return res.status(400).json({ success: false, message: "Token and MFA code are required" });
    }
    ...
    if (decoded.tenantSlug) {
      tenant = await Tenant.findOne({ slug: decoded.tenantSlug });
      if (tenant) {
        const db = await getTenantDB(tenant.dbName);
        req.tenantDB = db;
        User = getTenantModels(db).User;
    ...
    res.status(200).json({
      ...
      slug: tenant?.slug || null,
      isDbRefreshed,
      token: generateToken(user._id, tenant, user.tokenVersion || 0, decoded.sessionId),
    });
  } catch (error) {
